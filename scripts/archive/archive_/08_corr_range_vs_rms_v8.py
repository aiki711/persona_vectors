...
def load_and_normalize_range_csv(range_csv_path: str) -> pd.DataFrame:
    """
    入力CSVの形式に応じて、最低限この列を揃える：
      - tag, split, trait, range_lo, range_hi, sid(optional)
    モデル単位のCSVは sid="__ALL__" を付与して同一スキーマにする。
    """
    df = pd.read_csv(range_csv_path)

    # kind フィルタ（range / range_per_prompt どちらも通す）
    if "kind" in df.columns:
        k = df["kind"].astype(str).str.lower()
        df = df[k.str.contains("range", na=False)].copy()

    if df.empty:
        raise SystemExit("[ERROR] range_csv has no rows after kind filter. Check kind values.")

    # range_lo/hi を作る（入力に応じてマッピング）
    if "range_lo" not in df.columns:
        if "alpha_lo_median" in df.columns:
            df["range_lo"] = df["alpha_lo_median"]
        elif "alpha_lo_mean" in df.columns:
            df["range_lo"] = df["alpha_lo_mean"]
        else:
            raise SystemExit("[ERROR] No range_lo (or alpha_lo_*) columns found.")

    if "range_hi" not in df.columns:
        if "alpha_hi_median" in df.columns:
            df["range_hi"] = df["alpha_hi_median"]
        elif "alpha_hi_mean" in df.columns:
            df["range_hi"] = df["alpha_hi_mean"]
        else:
            raise SystemExit("[ERROR] No range_hi (or alpha_hi_*) columns found.")

    # 数値化
    df["range_lo"] = pd.to_numeric(df["range_lo"], errors="coerce")
    df["range_hi"] = pd.to_numeric(df["range_hi"], errors="coerce")

    # 必須列チェック
    for col in ["tag", "split", "trait"]:
        if col not in df.columns:
            raise SystemExit(f"[ERROR] Missing required column: {col}")

    # sid が無ければモデル単位とみなし __ALL__ 付与
    if "sid" not in df.columns:
        df["sid"] = "__ALL__"

    # 余計な欠損を落とす
    df = df.dropna(subset=["tag", "split", "trait", "range_lo", "range_hi"]).copy()
...
def build_merged_metrics(range_csv_path: str, jsonl_paths: List[str], rawnorm_map: Optional[Dict[tuple, Dict[str, float]]] = None) -> pd.DataFrame:
    df_range = load_and_normalize_range_csv(range_csv_path)

    known_tags = sorted(df_range["tag"].dropna().unique().tolist())
    if not known_tags:
        raise SystemExit("[ERROR] No tags found in range_csv (after normalization).")

    # range_map: (tag, split, trait, sid) -> row
    range_map: Dict[tuple, Any] = {}
    for _, row in df_range.iterrows():
        key = (str(row["tag"]), str(row["split"]), str(row["trait"]), str(row["sid"]))
        range_map[key] = row
    ...
    for fpath in jsonl_paths:
        ...
        # probe 側に sid が入ってる前提（無いなら __ALL__ 扱い）
        # ここは “1ファイルに複数sid” なので、process_jsonl_rms を sidごとに集計する必要がある。
        # いまの process_jsonl_rms はファイル全体平均なので、sid単位相関をしたいなら
        # 「sidごとに bucket → RMS特徴」を作る関数が別途必要です。
        #
        # まずはモデル単位(__ALL__)を壊さないために：sid="__ALL__" の range 行を使う。
        sid_key = "__ALL__"

        key = (tag, split, trait, sid_key)
        if key not in range_map:
            # sid関係なく（tag,split,trait一致の __ALL__ を探す）
            key = (tag, split, trait, "__ALL__")
            if key not in range_map:
                continue
...
def main():
    ...
    if args.rawnorm_npz_glob:
        ...
        if raw_paths:
            ...
            df_range_tmp = load_and_normalize_range_csv(args.range_csv)

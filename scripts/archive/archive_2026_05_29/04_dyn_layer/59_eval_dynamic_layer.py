...
# -*- coding: utf-8 -*-
#
# 59_eval_dynamic_layer.py
#
# Evaluates DLS (Dynamic Layer Selection) generated text using Llama-3 judge.
# Input:  dyn_Val{alpha}.jsonl  (contains base_text + dyn_text)
# Output: scores_dyn_Val{alpha}.csv
...
def main():
    ...
    ap.add_argument("--input",  required=True, help="Path to dyn_Val*.jsonl")
    ...
    for row in tqdm(data):
        b_score, b_reason = get_score(model, tokenizer, row["base_text"], args.axis, device)
        d_score, d_reason = get_score(model, tokenizer, row["dyn_text"],  args.axis, device)
        results.append({
            "idx":        row["idx"],
            "dyn_layer":  row["dyn_layer"],
            "base_score": b_score,
            "base_ppl":   row["base_ppl"],
            "dyn_score":  d_score,
            "dyn_ppl":    row["dyn_ppl"],
            "base_reason": b_reason.replace("\n", " "),
            "dyn_reason":  d_reason.replace("\n", " "),
        })

...
# -*- coding: utf-8 -*-
#
# 92_submit_eval_cos_prior.py
#
# Submits evaluation jobs for Cos-Prior DLS results.
...
echo "Starting evaluation for Cos-Prior {trait}..."

for val in {vals_list}; do
    JSONL_OUT="exp_steering_dyn_layer_proj_prior/results/{trait}/cos_prior_Val${{val}}.jsonl"
    CSV_OUT="exp_steering_dyn_layer_proj_prior/results/{trait}/scores_cos_prior_Val${{val}}.csv"
...
echo "Evaluation completed for Cos-Prior {trait}."
"""
...
def main():
    ...
    for trait in TRAITS:
        gen_job_name = f"dls_cos_prior_{trait}"
        dependency_line = ""
        if gen_job_name in active_jobs:
            job_id = active_jobs[gen_job_name]
            dependency_line = f"#SBATCH --dependency=afterok:{job_id}"
            print(f"Setting dependency for eval_cos_prior_{trait} on generation job {job_id} ({gen_job_name})")
        else:
        ...
        pbs_file = job_dir / f"run_eval_cos_prior_{trait}.sh"

...
# -*- coding: utf-8 -*-
#
# 79_plot_midpoint_vs_single_comparison.py
#
# DLS (全層) の結果と、DLS + IC-Adaptive Fusion (中点正規化: --norm_mode midpoint) の結果を
# 単層固定値αステアリングと比較するヒートマップを作成する。
#
# 比較手法:
#   - DLS Z-score logit_diff
#   - DLS Z-score anti_alignment
#   - DLS Relative Anti-alignment (DLS_Relative)
#   - Fusion Sigmoid (DLS_Relative + Sigmoid IC, Midpoint Normalized)
#   - Fusion Soft-Plateau (DLS_Relative + Soft-Plateau, Midpoint Normalized)
#
# Outputs:
#   - exp_steering_dyn_ic_fusion_midpoint/figures/{trait}/heatmap_{trait}_fusion_compare.png
#   - exp_steering_dyn_ic_fusion_midpoint/figures/summary_fusion_all_traits.png
#
...
def plot_axis(df: pd.DataFrame, axis: str, out_dir: Path, all_layers_dir: Path, fusion_dir: Path):
    print(f"\n[{axis}] processing midpoint-normalized comparison heatmap...")
    plt.close("all")
    ...
    # Reindex to ensure order of VALS
    p_c_score_comp = p_c_score_comp.reindex(VALS)
    p_a_score_comp = p_a_score_comp.reindex(VALS)
    p_c_ppl_comp   = p_c_ppl_comp.reindex(VALS)
    p_a_ppl_comp   = p_a_ppl_comp.reindex(VALS)
    ...
    for ax_obj, p_data, p_ppl_ref, title, cmap, vmin, vmax, fmt in configs:
        ...
        ax_obj.set_title(
            ...
            f" (Border:PPL<=25 | Navy/Red/Green: DLS | Orange/Purple: Midpoint Fusion)",
            fontsize=8, fontweight="bold")
        ax_obj.set_xlabel("Layer / Evaluation Variants (rightmost)")
        ax_obj.set_ylabel("Val (Steering Intensity)")

    plt.suptitle(f"DLS vs Midpoint Fusion Comparison Results: {axis.capitalize()}",
...
def make_summary_heatmaps(all_df: pd.DataFrame,
    ...
    if has_layer_data:
        ...
        for col, title, cmap, vmin, vmax, ax_obj in layer_configs:
            ...
            # Reindex rows to ensure order of VALS
            p = p.reindex(VALS)
            ...
            ax_obj.set_title(
                title + " (Border:PPL<=25 | Navy/Red/Green: DLS | Orange/Purple: Midpoint Fusion)",
                fontsize=8, fontweight="bold")
            ax_obj.set_xlabel("Layer / Evaluation Variants (rightmost)")
            ax_obj.set_ylabel("Val (Steering Intensity)")

        plt.suptitle("Layer-Sweep, DLS, and Midpoint Fusion Summary — All Traits Average",
                     fontsize=14, fontweight="bold", y=1.02)
    else:
        ...
        for dls_col, title, cmap, vmin, vmax, ax_obj in configs_dls:
            ...
            highlight_safe_cells(ax_obj, p_ppl, threshold=25.0)
            ax_obj.set_title(
                title + "\n(Border:PPL<=25 | Navy/Red/Green: DLS | Orange/Purple: Midpoint Fusion)",
                fontsize=8, fontweight="bold")
            ax_obj.set_xlabel("Evaluation Method")
            ax_obj.set_ylabel("Val (Steering Intensity)")
        plt.suptitle("DLS & Midpoint Fusion Summary — All Traits Average",
                     fontsize=14, fontweight="bold", y=1.02)

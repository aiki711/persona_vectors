...
# -*- coding: utf-8 -*-
#
# scripts/04_dyn_layer/122_submit_gen_time_eval.py
#
# Detects running/pending generation-time steering jobs in SLURM,
# and submits batch evaluation jobs with dependencies (`--dependency=afterok:<jobid>`)
# so they automatically run after the corresponding generation completes.
#
# Output: Evaluates files under exp_steering_dyn_gen_time_raw/results/{trait}/
#
...
#SBATCH --job-name=eval_gen_time_raw_{trait}
#SBATCH --partition=GPU-1
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:nvidia_a40:1
#SBATCH --time=04:00:00
#SBATCH --output=log/eval_gen_time_raw_{trait}.out
#SBATCH --error=log/eval_gen_time_raw_{trait}.err
...
echo "Starting batch evaluation for generation-time steering of {trait}..."
...
def main():
    job_dir = Path("jobs/eval_gen_time_raw")
    ...
    for trait in TRAITS:
        ...
        pbs_file = job_dir / f"run_eval_gen_time_{trait}.sh"
        with open(pbs_file, "w", encoding="utf-8") as f:
            f.write(pbs_content)
        pbs_file.chmod(0o755)

        # Detect dependency
        gen_job_name = f"gen_time_raw_{trait}"
        gen_job_id = None

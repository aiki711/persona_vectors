...
# -*- coding: utf-8 -*-
#
# 89_submit_proj_only.py
#
# Submits the Projection-Only DLS method sweeps to the SLURM queue.
...
#SBATCH --job-name=dls_proj_only_{trait}
#SBATCH --partition=GPU-1
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
#SBATCH --time=04:00:00
#SBATCH --output=log/dls_proj_only_{trait}.out
#SBATCH --error=log/dls_proj_only_{trait}.err
...
echo "Running Proj-Only DLS sweep for {trait}..."

# Loop over values and run the script with --no_prior flag
...
def main():
    job_dir = Path("jobs/dls_proj_only")
    ...
    for trait in TRAITS:
        ...
        pbs_file = job_dir / f"run_dls_proj_only_{trait}.sh"
        with open(pbs_file, "w") as f:
            f.write(pbs_content)
        pbs_file.chmod(0o755)

        cmd = ["sbatch", str(pbs_file)]
        print(f"Submitting Proj-Only DLS job for {trait}...")

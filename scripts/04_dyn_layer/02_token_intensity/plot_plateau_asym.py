...
# -*- coding: utf-8 -*-
#
# scripts/04_dyn_layer/02_token_intensity/plot_plateau_asym.py
#
...
RESULTS_DIR = WORKSPACE / "exp_token_intensity/exp_static_layer_plateau_asym/results"
FIGURES_DIR = WORKSPACE / "exp_token_intensity/exp_static_layer_plateau_asym/figures"
...
def main():
    print("Aggregating metrics for Plateau and Asymmetric Gating configurations...")
    ...
    # Print table values to stdout for console verification
    print("\nSteering Score Table:")
    print(f"{'Gating Config':<25} | {'DLS Rank':<10} | {'PDF Cos':<10} | {'PDF Rank':<10}")
    print("-" * 65)
    for idx, conf in enumerate(CONFIG_PARAMS.keys()):
        print(f"{conf:<25} | {score_matrix[0][idx]:.3f}   | {score_matrix[1][idx]:.3f}   | {score_matrix[2][idx]:.3f}")
        
    plt.ylabel('Steering Alignment Score (1.0 - 5.0)', fontsize=12, fontweight='bold', labelpad=10)
    plt.title('Steering Alignment Score by Plateau & Asymmetric Gating Configs', fontsize=13, fontweight='bold', pad=15)
    ...
    score_fig_path = FIGURES_DIR / "plateau_asym_score_comparison.png"
    ...
    print("\nText Perplexity Table:")
    print(f"{'Gating Config':<25} | {'DLS Rank':<10} | {'PDF Cos':<10} | {'PDF Rank':<10}")
    print("-" * 65)
    for idx, conf in enumerate(CONFIG_PARAMS.keys()):
        print(f"{conf:<25} | {ppl_matrix[0][idx]:.3f}   | {ppl_matrix[1][idx]:.3f}   | {ppl_matrix[2][idx]:.3f}")
        
    plt.ylabel('Text Perplexity (PPL)', fontsize=12, fontweight='bold', labelpad=10)
    plt.title('Text Perplexity by Plateau & Asymmetric Gating Configs', fontsize=13, fontweight='bold', pad=15)
    plt.xticks(x, x_labels, fontsize=9)
    plt.ylim(0, 20.0)
    plt.grid(axis='y', linestyle=':', alpha=0.6)
    ...
    ppl_fig_path = FIGURES_DIR / "plateau_asym_ppl_comparison.png"

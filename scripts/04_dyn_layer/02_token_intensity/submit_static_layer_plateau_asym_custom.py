...
# -*- coding: utf-8 -*-
#
# scripts/04_dyn_layer/02_token_intensity/submit_static_layer_plateau_asym_custom.py
#
...
OUT_DIR = WORKSPACE / "exp_token_intensity/exp_static_layer_plateau_asym_custom/results"
LOG_DIR = WORKSPACE / "log/static_layer_plateau_asym_custom"
JOBS_DIR = WORKSPACE / "jobs/static_layer_plateau_asym_custom"
...
CONFIGS = [
    # (conf_id, theta_lo, theta_hi, k_lo, k_hi, gating_mode)
...
#SBATCH --job-name=sl_pa_c_{prefix}_{trait}
#SBATCH --output={log_dir}/sl_pa_c_{prefix}_{trait}.log
#SBATCH --error={log_dir}/sl_pa_c_{prefix}_{trait}.err
...
if [ ! -f "{out_dir}/{trait}/{out_filename}" ]; then
...
def main():
    print("Generating Grouped Custom Plateau & Asymmetric SLURM submit scripts (2 hours walltime)...")
    for trait in TRAITS:
        for (score_mode, use_mask, prefix) in METHODS:
            script_path = JOBS_DIR / f"run_sl_pa_c_{prefix}_{trait}.sh"
            ...
            # Add steps for each of the 4 configs
    ...
    print("To submit a job, run: sbatch jobs/static_layer_plateau_asym_custom/run_sl_pa_c_<prefix>_<trait>.sh")

...
# -*- coding: utf-8 -*-
#
# 71_submit_dyn_all_midpoint.py
#
# 中点正規化（--norm_mode midpoint）を適用したZ-scoreベースライン（logit_diff / anti_alignment）の
# 実験ジョブを SLURM に投入するスクリプト。
#
# 実験フォルダ: exp_steering_dyn_layer_all_layers_midpoint/
#
...
#SBATCH --job-name=dls_calib_mid
#SBATCH --partition=GPU-1
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
#SBATCH --time=2:00:00
#SBATCH --output=log/dls_calib_mid.out
#SBATCH --error=log/dls_calib_mid.err
...
OUT_STATS="exp_steering_dyn_layer_all_layers_midpoint/dls_calibration_stats_all_midpoint.json"

mkdir -p "exp_steering_dyn_layer_all_layers_midpoint"

echo "Starting Midpoint-Normalized Calibration for all 32 layers..."
...
#SBATCH --job-name=dyn_mid_all_{trait}
#SBATCH --partition=GPU-1
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
#SBATCH --time=08:00:00
#SBATCH --output=log/dyn_mid_all_{trait}.out
#SBATCH --error=log/dyn_mid_all_{trait}.err
...
for V in "${{VALS[@]}}"; do
    ...
    echo "Running DLS Z-score logit_diff (Midpoint): Trait={trait}, Alpha=$V"
    ...
    echo "Running DLS Z-score anti_alignment (Midpoint): Trait={trait}, Alpha=$V"
...
def main():
    job_dir = Path("jobs/dyn_layer_all_midpoint")
    ...
    calib_file = job_dir / "run_calibration_all_midpoint.sh"
    with open(calib_file, "w") as f:
        f.write(calib_content)
    calib_file.chmod(0o755)

    print("Submitting midpoint calibration job...")
    ...
    for trait in TRAITS:
        ...
        eval_file = job_dir / f"run_all_midpoint_{trait}.sh"
        with open(eval_file, "w") as f:
            f.write(eval_content)
        eval_file.chmod(0o755)

        print(f"Submitting midpoint eval job for {trait} (dependency on {calib_job_id})...")
        res = subprocess.run(

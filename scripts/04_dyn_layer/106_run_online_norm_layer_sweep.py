...
# -*- coding: utf-8 -*-
#
# 106_run_online_norm_layer_sweep.py
#
# Single-layer steering sweep with midpoint-norm relative scaling (Pattern B).
# (||w_steered|| = alpha * (||w_raw|| / ||m||)).
#
# Output: exp_steering_layer_midpoint_norm/results/{axis}/layer_{L}_Val{alpha}.jsonl
#         exp_steering_layer_midpoint_norm/results/{axis}/scores_layer_{L}_Val{alpha}.csv
#
...
def main():
    ...
    # Load and scale steering vectors to midpoint-norm (Pattern B)
    layer_w = {}
    for L in LAYERS:
        ...
        if raw_norm_key in v_data and mp_key in v_data:
            r_norm = float(v_data[raw_norm_key][0])
            m_vec = torch.tensor(v_data[mp_key], dtype=torch.float32)
            m_norm = torch.norm(m_vec).item()
            
            # w_scaled = w * (raw_norm / m_norm)
            w_scaled = w_vec * (r_norm / (m_norm + 1e-10))
        else:
            print(f"  [WARNING] Missing raw_norm or midpoint for Layer {L}. Fallback to unit vector.")
            w_scaled = w_vec
    ...
    if missing_gens:
        print(f"=== Midpoint-Norm Single-Layer Sweep Generation ({args.axis}) ===")
        ...
        print("Steering vector norm (relative midpoint-norm scaling) per layer:")
        for L in sorted(layer_w.keys()):
            raw_w_norm = np.linalg.norm(v_data[f"{L}|{args.axis}|w"])
            r_norm = float(v_data[f"{L}|{args.axis}|raw_norm"][0]) if f"{L}|{args.axis}|raw_norm" in v_data else float("nan")
            scaled_norm = torch.norm(layer_w[L]).item()
            print(f"  Layer {L:2d}: raw_w_norm={raw_w_norm:.3f}, raw_norm={r_norm:.3f}, scaled_w_norm={scaled_norm:.5f}")
    ...
    if missing_evals:
        print(f"\n=== Midpoint-Norm Single-Layer Sweep Evaluation ({args.axis}) ===")
    ...
    print("\nDONE: All layers and values evaluated (midpoint-norm scaling).")

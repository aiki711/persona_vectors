...
def main():
    ...
    # Configurations to test (Entropy mode with alpha_max=5.0)
    # theta_lo serves as the entropy threshold, k_lo serves as the slope
    configs = [
        {"name": "Entropy-2.0", "theta_lo": 2.0, "k_lo": 1.5},
        {"name": "Entropy-3.0", "theta_lo": 3.0, "k_lo": 1.5},
        {"name": "Entropy-4.0", "theta_lo": 4.0, "k_lo": 1.5}
    ]
    ...
    for cfg in configs:
        ...
        # In entropy mode, theta_hi and k_hi are not used, but run_token_intensity_steering.py expects them in filenames
        # We can set dummy values for theta_hi and k_hi to avoid errors.
        t_hi = 7.0
        k_hi = 2.0
    ...
    summary_path = out_base_dir / "entropy_gating_summary.md"
    md_lines = [
        "# Predictive Entropy Gating Experiment Results",
        "\nThis report evaluates the **Predictive Entropy Gating** method which dynamically adjusts steering strength "
        "based on the Shannon entropy of the predicted token probability distribution to avoid steering lag.\n",
        "## 1. Overall Performance Table\n",
        "| Configuration | Entropy Threshold (theta_H) | Alignment Score (Target: >4.34) | Perplexity (PPL) (Target: <10.46) | Result |",
        "| :--- | :---: | :---: | :---: | :--- |"
    ]
    ...
    for res in sweep_results:
        ...
        if res["Score"] >= 4.34 and res["PPL"] < 10.46:
            ...
            status = "📉 Fluent but lower score"
            
        md_lines.append(f"| **{res['Config']}** | {res['Theta_H']} | **{res['Score']:.3f}** | **{res['PPL']:.3f}** | {status} |")
    ...
    with open(summary_path, "w", encoding="utf-8") as f:
        f.write(md_text)
    print(f"\nSaved entropy gating report to: {summary_path}")
    
    # Copy to artifacts
    artifact_dir = Path("/home/s2550009/.gemini/antigravity-ide/brain/6611299f-19cb-4461-bbfe-1854feeb8fae")
    shutil.copy(summary_path, artifact_dir / "entropy_gating_summary.md")

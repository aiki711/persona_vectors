...
# -*- coding: utf-8 -*-
#
# scratch/run_coordinate_step1.py
#
...
def main():
    ...
    # Step 1: Fix theta_lo = 1.2, sweep theta_hi at 0.1 intervals
    t_lo = 1.2
    theta_hi_vals = [5.5, 5.6, 5.7, 5.8, 5.9, 6.0, 6.1, 6.2]
    k_lo = 8.0
    k_hi = 8.0
    
    results = []
    
    total_runs = len(theta_hi_vals)
    current_run = 0
    
    print(f"Starting Coordinate Descent Step 1: Fixing theta_H = {t_lo}, Sweeping theta_IC...")
    
    for t_hi in theta_hi_vals:
        current_run += 1
        config_name = f"Dual-{t_lo}-{t_hi:.1f}"
        ...
        trait_ppls = []
        for trait in traits:
            csv_path = out_base_dir / trait / f"scores_masked_proj_rank_theta_{t_lo}_{t_hi:.1f}_k_{k_lo}_{k_hi}_dual_Val5.0.csv"
            # Fallback if float formatting in filename has no decimal point or different length
            # run_token_intensity_steering formatting: args.theta_hi is printed as is.
            # E.g. args.theta_hi = 6.0 is printed as 6.0.
            if not csv_path.exists():
                # Try formatting with single decimal place or trailing .0
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(md_text)
    print(f"\nSaved Step 1 report to: {report_path}")

...
#PBS -N scenario_exp
#PBS -q GPU-1
#PBS -o log/scenario_exp.o%j
#PBS -e log/scenario_exp.e%j
...
cd "$WORKDIR"
mkdir -p log
LOG_FILE="log/scenario_exp.${RUN_ID}.log"
exec > >(tee -a "$LOG_FILE") 2>&1

echo "=== STARTING SCENARIO EXPERIMENTS ==="
...
# ==================== Experiment Params ====================
# Using same layer range as main sweep for consistency
LAYER_START=10
LAYER_END=30
LAYER_SUFFIX="_L${LAYER_START}-${LAYER_END}"

# Helper to reuse same output root logic
EXP_OUTPUT_ROOT="exp${LAYER_SUFFIX}"

echo "LAYER CONFIG: ${LAYER_START} to ${LAYER_END}"
...
# New Scenario Prompt Sets
PROMPT_SETS=(
  "writing|exp/01_probe_inputs/writing_prompts_30.json|flexible"
  "advice|exp/01_probe_inputs/opinion_advice_30.json|flexible"
)
...
prepare_axes_if_needed() {
  local model_id="$1"
  local ax_bank="$2"
  local config_file="exp/configs/big5_vectors.yaml"
  
  if ! is_nonempty_file "$ax_bank"; then
      echo "[WARN] Axis bank $ax_bank missing. You might need to run main sweep first."
      # Just try running it
...
run_probe_if_needed() {
  ...
  echo "[RUN ] 01_run_probe.py -> $out_jsonl (Template: $template_type)"
...
run_viz_for_dir() {
    ...
    # Alpha vs Score/Distance Scatter
    ...
    # Combined Metrics (Slope Calculation and Radar Chart)
...
run_experiment_set() {
    ...
    echo ">>>>>>> START SCENARIO: $set_name (Layers ${LAYER_START}-${LAYER_END}) [Template: $template_type] <<<<<<<"
...
echo "=== GLOBAL ANALYSIS FOR SCENARIOS ==="
for pset in "${PROMPT_SETS[@]}"; do

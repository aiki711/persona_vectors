#!/bin/bash
#SBATCH --job-name=dyn_ic_extraversion
#SBATCH --partition=GPU-1
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
#SBATCH --time=24:00:00
#SBATCH --output=log/dyn_ic_extraversion.out
#SBATCH --error=log/dyn_ic_extraversion.err
...
for MODE in "${IC_MODES[@]}"; do
    for AMAX in "${AMAXES[@]}"; do
        echo "Running DLS + IC Fusion: Trait=extraversion, Mode=$MODE, AlphaMax=$AMAX"
        JSONL_OUT="${OUT_DIR}/extraversion/fusion_${MODE}_Val${AMAX}.jsonl"
        CSV_OUT="${OUT_DIR}/extraversion/scores_fusion_${MODE}_Val${AMAX}.csv"

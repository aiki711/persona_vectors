#!/bin/bash
#SBATCH --job-name=dyn_ic_neuroticism
#SBATCH --partition=GPU-1
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
#SBATCH --time=24:00:00
#SBATCH --output=log/dyn_ic_neuroticism.out
#SBATCH --error=log/dyn_ic_neuroticism.err
...
for MODE in "${IC_MODES[@]}"; do
    for AMAX in "${AMAXES[@]}"; do
        echo "Running DLS + IC Fusion: Trait=neuroticism, Mode=$MODE, AlphaMax=$AMAX"
        JSONL_OUT="${OUT_DIR}/neuroticism/fusion_${MODE}_Val${AMAX}.jsonl"
        CSV_OUT="${OUT_DIR}/neuroticism/scores_fusion_${MODE}_Val${AMAX}.csv"

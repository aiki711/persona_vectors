#!/bin/bash
#SBATCH --job-name=dyn_ic_agreeableness
#SBATCH --partition=GPU-1
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
#SBATCH --time=24:00:00
#SBATCH --output=log/dyn_ic_agreeableness.out
#SBATCH --error=log/dyn_ic_agreeableness.err
...
AMAXES=(5.0 10.0 15.0 20.0 25.0 30.0 35.0 40.0)
IC_MODES=(sigmoid soft_plateau)

for MODE in "${IC_MODES[@]}"; do
    for AMAX in "${AMAXES[@]}"; do
        echo "Running DLS + IC Fusion: Trait=agreeableness, Mode=$MODE, AlphaMax=$AMAX"
        JSONL_OUT="${OUT_DIR}/agreeableness/fusion_${MODE}_Val${AMAX}.jsonl"
        CSV_OUT="${OUT_DIR}/agreeableness/scores_fusion_${MODE}_Val${AMAX}.csv"

#!/bin/bash
#SBATCH --job-name=dyn_all_openness
#SBATCH --partition=GPU-1
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:1
#SBATCH --time=24:00:00
#SBATCH --output=log/dyn_all_openness.out
#SBATCH --error=log/dyn_all_openness.err
...
for V in "${VALS[@]}"; do
    ...
    echo "Running DLS Z-score logit_diff: Trait=openness, Alpha=$V"
    JSONL_OUT="${OUT_DIR}/openness/logit_diff_Val${V}.jsonl"
    CSV_OUT="${OUT_DIR}/openness/scores_logit_diff_Val${V}.csv"
    ...
    echo "Running DLS Z-score anti_alignment: Trait=openness, Alpha=$V"
    JSONL_OUT="${OUT_DIR}/openness/anti_alignment_Val${V}.jsonl"
    CSV_OUT="${OUT_DIR}/openness/scores_anti_alignment_Val${V}.csv"
    ...
    echo "Running DLS Relative Anti-alignment: Trait=openness, Alpha=$V"

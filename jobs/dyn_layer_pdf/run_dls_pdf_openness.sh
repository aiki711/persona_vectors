#!/bin/bash
#SBATCH --job-name=dls_pdf_openness
#SBATCH --partition=GPU-1
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:nvidia_a40:1
#SBATCH --time=06:00:00
#SBATCH --output=log/dls_pdf_openness.out
#SBATCH --error=log/dls_pdf_openness.err
...
echo "Starting text generation with PDF for openness..."
...
for val in 0.5 1.0 2.0 4.0 5.0 6.0 8.0 10.0 15.0 20.0 25.0 30.0 35.0 40.0; do
    ...
    JSONL_OUT="${OUT_DIR}/openness/masked_proj_cos_prior_Val${val}.jsonl"
    if [ ! -f "$JSONL_OUT" ]; then
        "$PYTHON_BIN" scripts/04_dyn_layer/82_run_dyn_layer_proj_prior.py \
            --config "$CONFIG" \
            --vector_bank "$VECTOR_BANK" \
    ...
    JSONL_OUT="${OUT_DIR}/openness/masked_proj_rank_prior_Val${val}.jsonl"
    if [ ! -f "$JSONL_OUT" ]; then
        "$PYTHON_BIN" scripts/04_dyn_layer/82_run_dyn_layer_proj_prior.py \
            --config "$CONFIG" \
            --vector_bank "$VECTOR_BANK" \
...
"$PYTHON_BIN" scripts/04_dyn_layer/88_batch_eval_pdf.py --axis openness --model "$JUDGE_MODEL"

echo "Completed DLS PDF run and evaluation for openness."

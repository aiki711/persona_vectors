#!/bin/bash
#SBATCH --job-name=eval_raw_fixed_seed_agreeableness
#SBATCH --partition=GPU-1
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:nvidia_a40:1
#SBATCH --time=12:00:00
#SBATCH --output=log/eval_raw_fixed_seed_agreeableness.out
#SBATCH --error=log/eval_raw_fixed_seed_agreeableness.err
...
echo "Starting evaluation on test prompts for agreeableness..."

for val in 0.5 1.0 2.0 4.0 5.0 6.0 8.0 10.0 15.0 20.0 25.0 30.0 35.0 40.0; do
    # ------------------ 1. UNMASKED METHODS ------------------
    # 1.1 Logit-Diff
    echo "=== Running Logit-Diff alpha=$val ==="
    JSONL_OUT="${OUT_DIR}/agreeableness/logit_diff_Val${val}.jsonl"
    CSV_OUT="${OUT_DIR}/agreeableness/scores_logit_diff_Val${val}.csv"
    if [ ! -f "$JSONL_OUT" ]; then
    ...
    JSONL_OUT="${OUT_DIR}/agreeableness/cos_only_Val${val}.jsonl"
    CSV_OUT="${OUT_DIR}/agreeableness/scores_cos_only_Val${val}.csv"
    if [ ! -f "$JSONL_OUT" ]; then
        "$PYTHON_BIN" scripts/04_dyn_layer/82_run_dyn_layer_steering.py \
            --config "$CONFIG" \
            --vector_bank "$VECTOR_BANK" \
    ...
    CSV_OUT="${OUT_DIR}/agreeableness/scores_rank_only_Val${val}.csv"
    if [ ! -f "$JSONL_OUT" ]; then
        "$PYTHON_BIN" scripts/04_dyn_layer/82_run_dyn_layer_steering.py \
            --config "$CONFIG" \
            --vector_bank "$VECTOR_BANK" \
    ...
    JSONL_OUT="${OUT_DIR}/agreeableness/proj_cos_only_Val${val}.jsonl"
    CSV_OUT="${OUT_DIR}/agreeableness/scores_proj_cos_only_Val${val}.csv"
    if [ ! -f "$JSONL_OUT" ]; then
        "$PYTHON_BIN" scripts/04_dyn_layer/82_run_dyn_layer_steering.py \
            --config "$CONFIG" \
            --vector_bank "$VECTOR_BANK" \
    ...
    JSONL_OUT="${OUT_DIR}/agreeableness/proj_rank_only_Val${val}.jsonl"
    CSV_OUT="${OUT_DIR}/agreeableness/scores_proj_rank_only_Val${val}.csv"
    if [ ! -f "$JSONL_OUT" ]; then
        "$PYTHON_BIN" scripts/04_dyn_layer/82_run_dyn_layer_steering.py \
            --config "$CONFIG" \
            --vector_bank "$VECTOR_BANK" \
    ...
    CSV_OUT="${OUT_DIR}/agreeableness/scores_masked_cos_only_Val${val}.csv"
    if [ ! -f "$JSONL_OUT" ]; then
        "$PYTHON_BIN" scripts/04_dyn_layer/82_run_dyn_layer_steering.py \
            --config "$CONFIG" \
            --vector_bank "$VECTOR_BANK" \
    ...
    CSV_OUT="${OUT_DIR}/agreeableness/scores_masked_rank_only_Val${val}.csv"
    if [ ! -f "$JSONL_OUT" ]; then
        "$PYTHON_BIN" scripts/04_dyn_layer/82_run_dyn_layer_steering.py \
            --config "$CONFIG" \
            --vector_bank "$VECTOR_BANK" \
    ...
    CSV_OUT="${OUT_DIR}/agreeableness/scores_masked_proj_cos_only_Val${val}.csv"
    if [ ! -f "$JSONL_OUT" ]; then
        "$PYTHON_BIN" scripts/04_dyn_layer/82_run_dyn_layer_steering.py \
            --config "$CONFIG" \
            --vector_bank "$VECTOR_BANK" \
    ...
    CSV_OUT="${OUT_DIR}/agreeableness/scores_masked_proj_rank_only_Val${val}.csv"
    if [ ! -f "$JSONL_OUT" ]; then
        "$PYTHON_BIN" scripts/04_dyn_layer/82_run_dyn_layer_steering.py \
            --config "$CONFIG" \
            --vector_bank "$VECTOR_BANK" \
...
echo "Evaluation completed on test prompts for agreeableness."

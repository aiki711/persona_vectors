#!/bin/bash
#SBATCH --job-name=eval_rank_only_neuroticism
...
echo "Starting evaluation for rank-Only neuroticism..."

for val in 0.5 1.0 2.0 4.0 5.0 6.0 8.0 10.0 15.0 20.0 25.0 30.0 35.0 40.0; do
    JSONL_OUT="exp_steering_dyn_layer_proj_prior/results/neuroticism/rank_only_Val${val}.jsonl"
    CSV_OUT="exp_steering_dyn_layer_proj_prior/results/neuroticism/scores_rank_only_Val${val}.csv"
    
    # Check both float version and integer version for paths
    if [ ! -f "$JSONL_OUT" ]; then
        # Check integer format fallback (e.g. Val5 instead of Val5.0)
        JSONL_OUT_INT="exp_steering_dyn_layer_proj_prior/results/neuroticism/rank_only_Val${val}.jsonl"
...
echo "Evaluation completed for rank-Only neuroticism."

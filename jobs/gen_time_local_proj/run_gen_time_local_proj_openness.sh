#!/bin/bash
#SBATCH --job-name=gen_time_local_proj_openness
#SBATCH --partition=GPU-1
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --gres=gpu:nvidia_a40:1
#SBATCH --time=03:00:00
#SBATCH --output=log/gen_time_local_proj_openness.out
#SBATCH --error=log/gen_time_local_proj_openness.err
...
echo "Starting generation-time Local Proj-Rank sweeps for openness..."

for val in 0.5 1.0 2.0 4.0 5.0 6.0 8.0 10.0 15.0 20.0 25.0 30.0 35.0 40.0; do
    # 1. Unmasked Local Proj-Rank
    JSONL_OUT="${OUT_DIR}/openness/local_proj_rank_only_Val${val}.jsonl"
    if [ ! -f "$JSONL_OUT" ]; then
    ...
    JSONL_OUT="${OUT_DIR}/openness/masked_local_proj_rank_only_Val${val}.jsonl"
    if [ ! -f "$JSONL_OUT" ]; then
...
echo "Generation-time Local Proj-Rank sweeps completed for openness."
